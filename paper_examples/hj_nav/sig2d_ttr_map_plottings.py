...
sig_map_ttr = np.load(f"{current_directory}/hj_values/sig2d_ttr_map.npy")
sig_general_ttr = np.load(f"{current_directory}/hj_values/sig2d_ttr_general.npy")
ttrs = [sig_map_ttr, sig_general_ttr]
ttr_names = ["sig_map_ttr", "sig_general_ttr"]  # 添加对应的名称列表

contour_levels = np.arange(0.0, 20.0, 0.5)
fixed_values = {2: 0.2, 3: math.pi/2}
sampling_state1 = np.array([2.0, 1.0])
sampling_state2 = np.array([2.0, 1.0])
sampling_states = np.vstack([sampling_state1, sampling_state2])
print(f"###### The estimated TTR heuristics at sampling states of the sig_map_ttrr is {grid.get_values(sig_map_ttr, sampling_states)} ######")
print(f"###### The estimated TTR heuristics at sampling states of the sig_general_ttr is {grid.get_values(sig_general_ttr, sampling_states)} ######")
# breakpoint()
