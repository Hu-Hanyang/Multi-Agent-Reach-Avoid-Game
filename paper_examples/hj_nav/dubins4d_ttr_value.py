...
po = PlotOptions(do_plot=True, plot_type="value", plotDims=[0,1],
                  slicesCut=[10, 18], save_fig=True, filename=f"{current_directory}/hj_plots/dubins4d_general_ttr.png")

# ttr_value = TTRSolver(my_car, grid, [goal_area, obs_area], tol, po)
ttr_value = TTRSolver(my_car, grid, reach_set, tol)


np.save(f"{current_directory}/hj_values/dubins4d_general_ttr.npy", ttr_value)
